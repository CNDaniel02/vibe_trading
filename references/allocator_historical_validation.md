...
外部 LLM、Exa、Robinhood 或 Alpaca，真实 broker write call 固定为 0。fixture
中的正 PnL 是测试数据，不是历史或 forward 盈利证据。
...
- 明确 `corporate_action_safe=true`。

期权 executable backtest 还要求当时完整 chain，并明确
`chain_complete=true`。每个合约需要 contract/chain identity、underlying、
...
截至 `2026-08-26T18:13:07Z` 的 48 小时输入：

- observed funnel：56 candidates、44 ranking、17 deep research、18 decisions、
  8 proposals、6 allocations、1 selected、1 paper order、1 fill；
- strict funnel：32 unique candidates、25 ranking、12 deep research、12 decisions、
  6 proposals、5 allocations、1 selected、1 observed paper order、1 fill；
- 24 份 snapshot 含 decision cutoff 后 observation，全部排除；admitted
...
- 历史输入缺 corporate-action-safe OHLCV 和完整 option chain，因此当前允许
  的历史结论只有 synthetic option sensitivity，不能报告 executable equity
  或 option historical PnL。
