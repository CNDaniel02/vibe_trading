...
10. 每个订单先注册 restart-safe mandate。`intraday_close` 当日退出，`next_close` 下一交易日退出，`two_to_five_days` 只持有指定 2-5 个交易日；缺失、矛盾、到期或 thesis invalidation 的 mandate fail closed 退出。
