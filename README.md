...
The OAuth token and client registration are stored only in a current-user DPAPI-encrypted file under `state/`. The audit verifies the complete 50-tool manifest. Runtime calls are explicitly limited to read-only equity quotes/historicals, option chains/instruments/quotes, and the earnings calendar.
